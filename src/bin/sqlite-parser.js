...
if (args["help"] || args._.length === 0) {
	...
	console.log(
		`--output [outfile]\t-o\tWrite output to a file instead of stdout`,
	);
...
stat(
	...
	checkThen(function ({ size }) {
		...
		if (output) {
			...
			stat(
				outDir,
				checkThen(startStream, function () {
...
function resolveArgs(argv) {
	...
	for (let i = 0; i < argv.length; i += 1) {
		...
		if (isNewArg(arg)) {
			const cur =
				arg.indexOf("--") !== -1 ? arg.slice(2) : aliases[arg.slice(1)];
...
function checkThen(resCallback = done, errCallback = error) {
...
function standardParser() {
	readFile(
		input,
		"utf8",
		checkThen(function (data) {
			parser(
				data,
				checkThen(function (ast) {
					let result;
					try {
