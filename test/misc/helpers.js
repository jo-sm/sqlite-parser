...
filePath = function (that, ext, glob = false) {
	var testTitle = (that.test ? that.test : that).title.trim(),
		...
		folderTitle = _.kebabCase(
			(that.test ? that.test : that).parent.title.trim(),
		);
	return (
		__dirname + "/../" + ext + "/" + folderTitle + "/" + fileTitle + "." + ext
	);
};
...
getTree = function (that) {
	return read(filePath(that, "sql"), "utf8").then(sqliteParser).then(broadcast);
};

getTestJson = function (that) {
	return read(filePath(that, "json"), "utf8").then(function (json) {
